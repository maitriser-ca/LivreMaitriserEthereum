...
((("Turing, Alan"))) Le nom fait référence au mathématicien anglais Alan Turing, considéré comme le père de l'informatique. En 1936, il défini un modèle mathématique représentant un ordinateur comme une machine à états qui manipule des symboles en les lisant et en les écrivant sur une mémoire séquentielle (ressemblant à une bande de papier de longueur infinie). C'est cette base mathématique que Turing a utilisé pour répondre (par la négative) aux questions de _calculabilité universelle_, c'est-à-dire savoir si tous problème peut être résolu par ordinateur. Il a prouvé qu'il existe des classes de problèmes qui ne sont pas calculables. ((("problème de l'arrêt"))) Il a notamment prouvé que le _problème de l'arrêt_  (savoir s'il est possible, étant donné un programme arbitraire et son entrée, de déterminer si ce programme finira par s'arrêter) n'est pas décidable.
...
Cependant, le Turing-complet est très dangereux, en particulier avec les systèmes à accès ouverts comme les blockchains publiques, à cause du problème de l'arrêt que nous avons évoqué précédemment. Par exemple, les imprimantes modernes sont Turing complètes et peuvent recevoir des fichiers à imprimer qui sont utilisés tel quel. Du fait que l'Ethereum soit Turing complet signifie qu'également n'importe quel programme de n'importe quelle complexité peut être calculé par Ethereum. Mais cette flexibilité pose des problèmes délicat de sécurité et de gestion des ressources. Une imprimante qui ne répond pas peut être éteinte et rallumée. Hors cela n'est pas possible avec une blockchain publique.
